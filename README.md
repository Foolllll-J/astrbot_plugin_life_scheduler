...
| 指令 | 说明 |
| :--- | :--- |
| `查看日程` / `/life show` | 查看 Bot 今日的日程安排和穿搭。 |
| `重写日程` / `/life renew` | 强制重新生成今日日程（会覆盖已有数据，消耗 LLM Token）。 |
| `重写日程 <补充要求>` | 带补充要求重新生成日程。LLM 会在生成时特别考虑你的要求。 |
| `日程时间 <HH:MM>` / `/life time` | 设置每日自动生成日程的时间。 |

### 💡 补充要求示例

`重写日程` 命令支持附加自定义要求，让生成的日程更符合你的期望：

```
重写日程 今天穿包臀皮裙和渔网袜，内容具体一点
重写日程 下午安排一个咖啡厅约会
重写日程 今天是宅家日，不要出门
```

LLM 会在保持人设一致性的前提下，将你的补充要求融入日程生成。
